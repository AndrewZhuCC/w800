...
//  mp3DecInfo = (MP3DecInfo *)uni_malloc(sizeof(MP3DecInfo));
//  if (!mp3DecInfo)
//    return 0;
//  ClearBuffer(mp3DecInfo, sizeof(MP3DecInfo));
//  
//  fh =  (FrameHeader *)     uni_malloc(sizeof(FrameHeader));
//  si =  (SideInfo *)        uni_malloc(sizeof(SideInfo));
//  sfi = (ScaleFactorInfo *) uni_malloc(sizeof(ScaleFactorInfo));
//  hi =  (HuffmanInfo *)     uni_malloc(sizeof(HuffmanInfo));
//  di =  (DequantInfo *)     uni_malloc(sizeof(DequantInfo));
//  mi =  (IMDCTInfo *)       uni_malloc(sizeof(IMDCTInfo));
//  sbi = (SubbandInfo *)     uni_malloc(sizeof(SubbandInfo));

	mp3DecInfo = (MP3DecInfo *)share_mem_get_addr();
	if (!mp3DecInfo)
		return 0;
	ClearBuffer(mp3DecInfo, sizeof(MP3DecInfo));
	fh =  (FrameHeader *)     ((char *)mp3DecInfo + sizeof(MP3DecInfo));
	si =  (SideInfo *)        ((char *)fh + sizeof(FrameHeader));
	sfi = (ScaleFactorInfo *) ((char *)si + sizeof(SideInfo));
	hi =  (HuffmanInfo *)     ((char *)sfi + sizeof(ScaleFactorInfo));
	di =  (DequantInfo *)     ((char *)hi + sizeof(HuffmanInfo));
	mi =  (IMDCTInfo *)       ((char *)di + sizeof(DequantInfo));
	sbi = (SubbandInfo *)     ((char *)mi + sizeof(IMDCTInfo));
...
#if 0
      printf("MP3DecInfo Size: %lu\n", sizeof(MP3DecInfo));
      printf("FrameHeader Size: %lu\n", sizeof(FrameHeader));
      printf("SideInfo Size: %lu\n", sizeof(SideInfo));
      printf("ScaleFactorInfo Size: %lu\n", sizeof(ScaleFactorInfo));
      printf("HuffmanInfo Size: %lu\n", sizeof(HuffmanInfo));
      printf("DequantInfo Size: %lu\n", sizeof(DequantInfo));
      printf("IMDCTInfo Size: %lu\n", sizeof(IMDCTInfo));
      printf("SubbandInfo Size: %lu\n", sizeof(SubbandInfo));
      printf("Decoder total Size: %lu\n", \
              sizeof(MP3DecInfo) + sizeof(FrameHeader) + sizeof(SideInfo) + sizeof(ScaleFactorInfo) \
...
#define SAFE_FREE(x)	{if (x)	uni_free(x);	(x) = 0;}	/* helper macro */
...
static FrameHeader _frameHeader;
static SideInfo _sideInfo;
static ScaleFactorInfo _scaleFactorInfo;
static HuffmanInfo _huffmanInfo;
static DequantInfo _dequantInfo;
static IMDCTInfo _iMDCTinfo;
static SubbandInfo _subbandInfo;
static MP3DecInfo _mp3DecInfo;

MP3DecInfo *InitBuffers(void)
{
    MP3DecInfo *mp3DecInfo;
    FrameHeader *fh;
    SideInfo *si;
