...
void PolyphaseMono(short *pcm, int *vbuf, const int *coefBase)
{
	...
	*(pcm + 16) = ClipToShort(dext_20(sum1L), DEF_NFRACBITS);
	...
	for (i = 15; i > 0; i--) {
		...
		*(pcm)       = ClipToShort(dext_20(sum1L), DEF_NFRACBITS);
        //*(pcm) = ClipToShort6((int)SAR64(sum1L, (32-CSHIFT)));
        //*(pcm) = ClipToShort6(dext_20(sum1L));
        //*(pcm) = sum1L;
#if 1
		//*(pcm + 2*i) = ClipToShort((int)SAR64(sum2L, (32-CSHIFT)), DEF_NFRACBITS);
		*(pcm + 2*i) = ClipToShort(dext_20(sum2L), DEF_NFRACBITS);
        //*(pcm + 2*i) = ClipToShort6((int)SAR64(sum2L, (32-CSHIFT)));
        //*(pcm + 2*i) = ClipToShort6(dext_20(sum2L));
        //*(pcm + 2 * i) = sum2L;
#endif
		//pcm++;
	}
