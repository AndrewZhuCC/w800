...
static InterruptHandle InterruptCreate() {
  Interruptable *interrupter = (Interruptable *)uni_malloc(sizeof(Interruptable));
  uni_sem_init(&interrupter->v, 0);
...
static int InterruptableSleep(InterruptHandle handle, int sleep_msec) {
  Interruptable *interrupter = (Interruptable *)handle;
  return uni_sem_wait_ms(interrupter->v, sleep_msec);
}

static int InterruptableBreak(InterruptHandle handle) {
  Interruptable *interrupter = (Interruptable *)handle;
  uni_sem_post(interrupter->v);
  return 0;
}
//----------------UTILS interruptable sleep--------------------
...
static void _payload_len_crc16_set(CommProtocolPacket *packet) {
  uint16_t checksum = _crc16((const char *)packet->payload_len, sizeof(packet->payload_len));
...
static void _checksum_calc(CommProtocolPacket *packet) {
  ...
  packet->checksum[1] = 0;
  uint16_t checksum = _crc16((const char*)packet, _packet_len_get(packet));
...
static uni_bool _is_payload_len_crc16_valid(CommPayloadLen length, CommChecksum crc) {
  ...
  uint16_t length_crc = _crc16((const char *)len, sizeof(len));
  if (crc != length_crc) {
