...
static void us615_lprstr(int argc, char **argv)
{
    u8 channel;
    u8 bandwidth = 0;
    ...
    if (argc > 2 || argc < 1)
    {
        printf("Argument failed\n"
               "us615 &lprstr <channel> [bandwidth]\n");
        return;
    }
...
static void us615_lprstp(int argc, char **argv)
{
    if (argc != 0)
    {
        printf("Argument failed\n"
...
static void us615_lprstt(int argc, char **argv)
{
    u32 cnt_total = 0;
    u32 cnt_good = 0;
    u32 cnt_bad = 0;
...
static void us615_lpchl(int argc, char **argv)
{
    u8 channel;
    u8 bandwidth = 0;
...
static void us615_lptpd(int argc, char **argv)
{
    u32 period;
...
static void us615_lptstr(int argc, char **argv)
{
    u32 tempcomp;
    u32 packetcnt;
    u16 psdulen;
    ...
    {
        printf("Argument failed\n"
               "us615 &lptstr <tempcomp> <count> <length> <gain> <rate>\n");
        return;
    }
...
static void us615_lptstp(int argc, char **argv)
{
    if (argc != 0)
    {
        printf("Argument failed\n"
...
static void us615_regr(int argc, char **argv)
{
    int i;
    u32 addr;
    u32 cnt;
...
static void us615_regw(int argc, char **argv)
{
    int i;
    u32 cnt;
    u32 addr;
    ...
    {
        printf("Argument failed\n"
               "us615 &regw <addr> <value1> [value2]...[value8]\n");
        return;
    }
...
static const char* rf_help_str = "us615 &lprstr:set rx channel\n"
                       "us615 &lprstp:set rx process\n"
                       "us615 &lprstt:get rx result\n"
                       "us615 &lpchl:set tx channel\n"
                       "us615 &lptpd:packet sending interval\n"
                       "us615 &lptstr:start tx process\n"
                       "us615 &lptstp:stop tx process\n"
                       "us615 &regr:read register\n"
                       "us615 &regw:write register\n\n>";

static void us615_rf_cli_cmd(char *wbuf, int wbuf_len, int argc, char **argv)
{
    if (argc < 2) {
        goto help;
    }
    if (strcmp(argv[1], "&lprstr") == 0)
        us615_lprstr(argc - 2, argv + 2);
    else if (strcmp(argv[1], "&lprstp") == 0)
        us615_lprstp(argc - 2, argv + 2);
    else if (strcmp(argv[1], "&lprstt") == 0)
        us615_lprstt(argc - 2, argv + 2);
    else if (strcmp(argv[1], "&lpchl") == 0)
        us615_lpchl(argc - 2, argv + 2);
    else if (strcmp(argv[1], "&lptpd") == 0) {
        us615_lptpd(argc - 2, argv + 2); 
    } else if (strcmp(argv[1], "&lptstr") == 0)
        us615_lptstr(argc - 2, argv + 2);
    else if (strcmp(argv[1], "&lptstp") == 0)
        us615_lptstp(argc - 2, argv + 2);
    else if (strcmp(argv[1], "&regr") == 0)
        us615_regr(argc - 2, argv + 2);
    else if (strcmp(argv[1], "&regw") == 0)
        us615_regw(argc - 2, argv + 2);
    else
        goto help;
...
static void us615_wifi_debug(int argc, char **argv)
{
    u8* tx_gain;
...
static char *scan_privacy_string(u8 privacy)
{
    ...
    switch (privacy)
    {
    case UNI_WIFI_AUTH_MODE_OPEN:
        sec = "NONE";
        break;
    case UNI_WIFI_AUTH_MODE_WEP_AUTO:
        sec = "WEP/AUTO";
        break;
    case UNI_WIFI_AUTH_MODE_WPA_PSK_TKIP:
        sec = "WPA_PSK/TKIP";
        break;
    case UNI_WIFI_AUTH_MODE_WPA_PSK_CCMP:
        sec = "WPA_PSK/CCMP";
        break;
    case UNI_WIFI_AUTH_MODE_WPA_PSK_AUTO:
        sec = "WPA_PSK/AUTO";
        break;
    case UNI_WIFI_AUTH_MODE_WPA2_PSK_TKIP:
        sec = "WPA2_PSK/TKIP";
        break;
    case UNI_WIFI_AUTH_MODE_WPA2_PSK_CCMP:
        sec = "WPA2_PSK/CCMP";
        break;
    case UNI_WIFI_AUTH_MODE_WPA2_PSK_AUTO:
        sec = "WPA2_PSK/AUTO";
        break;
    case UNI_WIFI_AUTH_MODE_WPA_WPA2_PSK_TKIP:
        sec = "WPA_PSK/WPA2_PSK/TKIP";
        break;
    case UNI_WIFI_AUTH_MODE_WPA_WPA2_PSK_CCMP:
        sec = "WPA_PSK/WPA2_PSK/CCMP";
        break;
    case UNI_WIFI_AUTH_MODE_WPA_WPA2_PSK_AUTO:
...
static void us615_wifi_scan(int argc, char **argv)
{
    if (argc != 0)
    {
        printf("Argument failed\n"
...
static void us615_wifi_cli_cmd(char *wbuf, int wbuf_len, int argc, char **argv)
{
    if (argc < 2) {
        goto help;
    }
    if (strcmp(argv[1], "wifi_debug") == 0)
        us615_wifi_debug(argc - 2, argv + 2);
    else if (strcmp(argv[1], "wifi_scan") == 0)
        us615_wifi_scan(argc - 2, argv + 2);
    else
        goto help;
