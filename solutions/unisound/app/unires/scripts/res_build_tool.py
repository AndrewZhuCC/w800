...
PCM_BIN = "./pcm.bin"
EXT_FLASH_PCM_BASE_ADDR = 0x1000L  # 必须是4K的整数倍
EXT_FLASH_PCM_BIN = "./ext_flash_pcm.bin"
EXT_FLASH_MP3_BIN = "./ext_flash_mp3.bin"
TARGET_PCM_ARRAY_FILE = "../../include/uni_pcm_array.h"
TARGET_PCM_DTAT_FILE = "../../include/pcm_bin.h"
TARGET_MP3_DTAT_FILE = "../../include/mp3_bin.h"

ASRFIX_DAT = "./lp_asrfix.dat"
...
TARGET_GRAMMAR_DATA_FILE = "../../include/kws_grammar.h"
...
def _copy_default_tons():
    file_list = _get_tone_filelist(DEFAULT_PCM_DIR)
    ...
    for home, dirs, files in os.walk(WAV_PCM_DIR):
        for filename in files:
            ...
            if wav_file[1] == ".wav":
                ...
                os.system("lame --silent --add-id3v2 -b 8 " + wav_file_path + " " + mp3_file_path)
...
def _wav_2_pcm_tons():
    ...
    for home, dirs, files in os.walk(WAV_PCM_DIR):
        for filename in files:
            wav_file = os.path.splitext(filename)
            if wav_file[1] == ".wav":
                pcm_file = wav_file[0] + ".pcm"
                wav_file_path = os.path.join(WAV_PCM_DIR, filename)
                pcm_file_path = os.path.join(PCM_DIR, pcm_file)
                with open(wav_file_path, 'rb') as in_file:
                    in_file.seek(44)
                    data = in_file.read()
                with open(pcm_file_path, 'wb') as out_file:
                    out_file.write(data)
                os.system('./adpcm_tool -i ' + pcm_file_path)
    ...
    for file in file_list:
        ...
        if file_name[1] == ".mp3":
            ...
            with open(file, 'rb') as pcm_file:
                default_pcm_content += pcm_file.read()
            fill_data_len = 64 - (file_size % 64)  # 64byte align
            default_pcm_content += bytearray(fill_data_len)
            file_index = int(file_name[0].upper())
            inc_file.write("  {" + str(file_index) + ", " + hex(flash_addr) + ", " + hex(file_size) + "},\n")
            flash_addr += (file_size + fill_data_len)
    inc_file.write("};\n")
    inc_file.write("\n")
    inc_file.write("#endif\n")
    inc_file.close()
    with open(PCM_BIN, "wb") as pcm_bin_fd:
        pcm_bin_fd.write(default_pcm_content)
...
def _create_grammar_array(ctrl):
    _bin_2_array(GRAMMAR_DATA, "grammar", TARGET_GRAMMAR_DATA_FILE)
...
def _modify_aik_json_file(aik_json):
    ...
    try:
        ...
        print(AIK_CONFIG + ' prase err:' + str(e))
    json.dump(cfg_bin_json, cfg_bin_file, indent=2, separators=(',', ': '), encoding="utf-8", ensure_ascii=False,
              sort_keys=False)
    cfg_bin_file.close()
...
def _get_internal_reply_str(cmd):
    if cmd == 'wakeup_uni':
        return '[0]'
    elif cmd == 'exitUni' or cmd == 'asr_timeout_uni':
        return '[2]'
    else:
        return '[1]'
