...
int32_t csi_i2s_config(i2s_handle_t handle, i2s_config_t *config)
{
    ...
    {
    	uni_i2s_set_format(I2S_Standard);
    }
    else if (config->cfg.protocol == I2S_PROTOCOL_MSB_JUSTIFIED)
    {
    	uni_i2s_set_format(I2S_Standard_MSB);
    }
    else if (config->cfg.protocol == I2S_PROTOCOL_PCM)
    {
    	uni_i2s_set_format(I2S_Standard_PCMA);//I2S_Standard_PCMB
    }

    if (config->cfg.width == I2S_SAMPLE_16BIT)
    {
        uni_i2s_set_word_len(I2S_DataFormat_16);
    }
    else if (config->cfg.width == I2S_SAMPLE_24BIT)
    {
        uni_i2s_set_word_len(I2S_DataFormat_24);
    }
    else if (config->cfg.width == I2S_SAMPLE_32BIT)
    {
        uni_i2s_set_word_len(I2S_DataFormat_32);
    }
    ...
    uni_i2s_set_freq(value, config->cfg.mclk_freq);

    if (config->cfg.tx_mono_enable || config->cfg.rx_mono_enable)
        uni_i2s_mono_select(1);
    else
        uni_i2s_mono_select(0);

    if (config->cfg.rx_mono_select_ch == I2S_RX_RIGHT_CHANNEL)
        uni_i2s_left_channel_sel(I2S_RIGHT_CHANNEL);
    else
        uni_i2s_left_channel_sel(I2S_LEFT_CHANNEL);
...
uint32_t csi_i2s_send(i2s_handle_t handle, const uint8_t *data, uint32_t length)
{
    ...
    priv->tx_runing = 1;
    ret = uni_i2s_tx_int((int16_t *)data, length / 4, NULL);
    priv->tx_runing = 0;
...
uint32_t csi_i2s_receive(i2s_handle_t handle, uint8_t *buf, uint32_t length)
{
    ...
    priv->rx_runing = 1;
    ret = uni_i2s_rx_int((int16_t *)buf, length / 4);
    priv->rx_runing = 0;
