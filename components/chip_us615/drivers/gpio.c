                                 ...
                                 gpio_mode_e mode)
{
    ...
    if (GPIO_MODE_PULLNONE == mode)
        tls_gpio_cfg_attr(gpio_pin_priv->pinidx, UNI_GPIO_ATTR_FLOATING);
    else if (GPIO_MODE_PULLUP == mode)
        tls_gpio_cfg_attr(gpio_pin_priv->pinidx, UNI_GPIO_ATTR_PULLHIGH);
    else if (GPIO_MODE_PULLDOWN == mode)
        tls_gpio_cfg_attr(gpio_pin_priv->pinidx, UNI_GPIO_ATTR_PULLLOW);
    else if (GPIO_MODE_OPEN_DRAIN == mode)
        tls_io_cfg_set(gpio_pin_priv->pinidx, UNI_IO_OPTION5);
...
int32_t csi_gpio_pin_config_direction(gpio_pin_handle_t handle, gpio_direction_e dir)
{
    ...
    if (GPIO_DIRECTION_INPUT == dir)
        tls_gpio_cfg_dir(gpio_pin_priv->pinidx, UNI_GPIO_DIR_INPUT);
    else if (GPIO_DIRECTION_OUTPUT == dir)
        tls_gpio_cfg_dir(gpio_pin_priv->pinidx, UNI_GPIO_DIR_OUTPUT);
    else
        return ERR_GPIO(DRV_ERROR_PARAMETER);
...
int32_t csi_gpio_pin_set_irq(gpio_pin_handle_t handle, gpio_irq_mode_e mode, bool enable)
{
    ...
    if (enable)
    {
        ...
        if (GPIO_IRQ_MODE_RISING_EDGE == mode)
            tls_gpio_irq_enable(gpio_pin_priv->pinidx, UNI_GPIO_IRQ_TRIG_RISING_EDGE);
        else if (GPIO_IRQ_MODE_FALLING_EDGE == mode)
            tls_gpio_irq_enable(gpio_pin_priv->pinidx, UNI_GPIO_IRQ_TRIG_FALLING_EDGE);
        else if (GPIO_IRQ_MODE_DOUBLE_EDGE == mode)
            tls_gpio_irq_enable(gpio_pin_priv->pinidx, UNI_GPIO_IRQ_TRIG_DOUBLE_EDGE);
        else if (GPIO_IRQ_MODE_LOW_LEVEL == mode)
            tls_gpio_irq_enable(gpio_pin_priv->pinidx, UNI_GPIO_IRQ_TRIG_LOW_LEVEL);
        else if (GPIO_IRQ_MODE_HIGH_LEVEL == mode)
            tls_gpio_irq_enable(gpio_pin_priv->pinidx, UNI_GPIO_IRQ_TRIG_HIGH_LEVEL);
        else
            return ERR_GPIO(DRV_ERROR_PARAMETER);
    }
    else
