...
#define  uni_i2c_int_mask(bl)   do { tls_bitband_write(HR_I2C_CTRL, 6, bl);}while(0);
 ...
 * @note           None
 */
void uni_i2c_transfer_done_register(void (*done)(void));
 ...
 *	- \ref UNI_SUCCESS
 */
int uni_i2c_start_write_it(uint8_t devaddr, uint8_t wordaddr, uint8_t * buf, uint16_t len);
 ...
 */
int uni_i2c_start_read_it(uint8_t devaddr, uint8_t wordaddr, uint8_t * buf, uint16_t len);
/**
 * @}
 */
