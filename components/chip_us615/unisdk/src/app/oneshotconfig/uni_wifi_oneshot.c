...
#include "uni_wifi.h"
#include "uni_wifi_oneshot.h"
#include "utils.h"
#include "uni_params.h"
#include "uni_osal.h"
#include "tls_wireless.h"
#include "uni_wl_task.h"
#include "uni_webserver.h"
#include "uni_timer.h"
#include "uni_cpu.h"
#include "uni_oneshot_lsd.h"
#include "uni_efuse.h"
...
extern tls_bt_status_t uni_bt_wifi_cfg_init(void);
extern tls_bt_status_t uni_bt_wifi_cfg_deinit(void);
#endif
...
void tls_wifi_set_oneshot_customdata(u8 *data)
{
	...
	if (gpfResult)
	{
		gpfResult(UNI_WIFI_ONESHOT_TYPE_CUSTOMDATA);	
	}
}
...
void uni_oneshot_net_status_event(uint32_t event_id, const void *data, void *context)
{
    if (oneshot_msg_q)
	{
...
{
    if (gpfResult)
        gpfResult(UNI_WIFI_ONESHOT_TYPE_SSIDPWD);
#if TLS_CONFIG_AP_MODE_ONESHOT	
	if((2 == guconeshotflag)||(3 == guconeshotflag))
	{
...
#endif	
	{	
		//tls_netif_add_status_event(uni_oneshot_netif_status_event);
		event_subscribe(EVENT_NET_GOT_IP, uni_oneshot_net_status_event, NULL);
	}
...
int soft_ap_create(void)
{
	...
	//printf("\n ap create %s ! \n", (ret == UNI_SUCCESS)? "Successfully" : "Error");
...
#endif
#if 0
void uni_oneshot_netif_status_event(u8 status )
{
...
void uni_oneshot_send_mac(void)
{
	int idx;
	int socket_num = 0;
	u8 mac_addr[8];
...
void tls_oneshot_task_handle(void *arg)
{
    ...
    for(;;)
    {
        ...
        switch((u32)msg)
        {
           ...
           //tls_netif_remove_status_event(uni_oneshot_netif_status_event);
           event_unsubscribe(EVENT_NET_GOT_IP, uni_oneshot_net_status_event, NULL);
	...
	if (NULL == oneshot_msg_q)
	{
		...
		if (OneshotTaskStk)
		{
			err = tls_os_task_create(NULL, NULL,
					...
					NULL,
					(void *)OneshotTaskStk, 		 /* ����ջ����ʼ��ַ */
					ONESHOT_TASK_SIZE * sizeof(u32), /* ����ջ�Ĵ�С	   */
...
#if TLS_CONFIG_AP_MODE_ONESHOT
		//tls_netif_add_status_event(uni_oneshot_netif_status_event);
        //event_subscribe(EVENT_NET_GOT_IP, uni_oneshot_netif_status_event, NULL);
		soft_ap_create();
#endif
	}
...
#if TLS_CONFIG_BT
__attribute__((weak)) tls_bt_status_t uni_bt_wifi_cfg_init()
{
	return TLS_BT_STATUS_FAIL;
}
__attribute__((weak)) tls_bt_status_t uni_bt_wifi_cfg_deinit()
{
	return TLS_BT_STATUS_FAIL;
}
#endif
