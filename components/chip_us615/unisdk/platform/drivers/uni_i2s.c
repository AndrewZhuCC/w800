...
uni_i2s_buf_t  uni_i2s_buf[1] = { 0 };
static uint8_t tx_channel = 0, rx_channel = 0;
static uni_dma_desc g_dma_desc_tx[2];
static uni_dma_desc g_dma_desc_rx[2];

//master or slave
void uni_i2s_set_mode(bool bl)
{
...
#if 0
//master or slave
static uint32_t uni_i2s_get_mode(void)
{
	uint32_t reg;
...
//i2s_stardard
void uni_i2s_set_format(uint32_t format)
{
	uint32_t reg;
	reg = tls_reg_read32(HR_I2S_CTRL);
	reg &= ~(0x3<<24);
...
void uni_i2s_left_channel_sel(bool bl)
{
	tls_bitband_write(HR_I2S_CTRL, 23, bl);
}

void uni_i2s_mono_select(bool bl)
{
	tls_bitband_write(HR_I2S_CTRL, 22, bl);
}

void uni_i2s_rx_dma_enable(bool bl)
{
	tls_bitband_write(HR_I2S_CTRL, 21, bl);
}

void uni_i2s_tx_dma_enable(bool bl)
{
	tls_bitband_write(HR_I2S_CTRL, 20, bl);
}

void uni_i2s_rx_fifo_clear()
{
	tls_bitband_write(HR_I2S_CTRL, 19, 1);
}

void uni_i2s_tx_fifo_clear()
{
	tls_bitband_write(HR_I2S_CTRL, 18, 1);
}

#if 0
static void uni_i2s_left_zerocross_enable(bool bl)
{
	tls_bitband_write(HR_I2S_CTRL, 17, bl);
}

static void uni_i2s_right_zerocross_enable(bool bl)
{
	tls_bitband_write(HR_I2S_CTRL, 16, bl);
}
#endif

void uni_i2s_set_rxth(uint8_t th)
{
	uint32_t reg;
...
void uni_i2s_set_txth(uint8_t th)
{
	uint32_t reg;
...
#if 0
static void uni_i2s_clk_inverse(bool bl)
{
...
void uni_i2s_set_word_len(uint8_t len)
{
	uint32_t reg;
	reg = tls_reg_read32(HR_I2S_CTRL);
	reg &= ~(0x3<<4);
...
#if 0
static void uni_i2s_set_mute(bool bl)
{
	tls_bitband_write(HR_I2S_CTRL, 3, bl);
}
#endif

void uni_i2s_rx_enable(bool bl)
{
	tls_bitband_write(HR_I2S_CTRL, 2, bl);
}

void uni_i2s_tx_enable(bool bl)
{
	tls_bitband_write(HR_I2S_CTRL, 1, bl);
}

void uni_i2s_enable(bool bl)
{
	tls_bitband_write(HR_I2S_CTRL, 0, bl);
}

#if 0
static void uni_i2s_lzc_int_mask(bool bl)
{
	tls_bitband_write(HR_I2S_INT_MASK, 9, bl);
}

static void uni_i2s_rzc_int_mask(bool bl)
{
	tls_bitband_write(HR_I2S_INT_MASK, 8, bl);
}

static void uni_i2s_txdone_int_mask(bool bl)
{
	tls_bitband_write(HR_I2S_INT_MASK, 7, bl);
}
#endif

static void uni_i2s_txth_int_mask(bool bl)
{
	tls_bitband_write(HR_I2S_INT_MASK, 6, bl);
}

#if 0
static void uni_i2s_txover_int_mask(bool bl)
{
	tls_bitband_write(HR_I2S_INT_MASK, 5, bl);
}

static void uni_i2s_txuderflow_int_mask(bool bl)
{
	tls_bitband_write(HR_I2S_INT_MASK, 4, bl);
}

static void uni_i2s_rxdone_int_mask(bool bl)
{
	tls_bitband_write(HR_I2S_INT_MASK, 3, bl);
}
#endif

static void uni_i2s_rxth_int_mask(bool bl)
{
	tls_bitband_write(HR_I2S_INT_MASK, 2, bl);
}

#if 0
static void uni_i2s_rx_overflow_int_mask(bool bl)
{
	tls_bitband_write(HR_I2S_INT_MASK, 1, bl);
}

static void uni_i2s_rx_uderflow_int_mask(bool bl)
{
	tls_bitband_write(HR_I2S_INT_MASK, 0, bl);
}
#endif

void uni_i2s_set_freq(uint32_t lr_freq, uint32_t mclk) 
{
...
#if 0
static void uni_i2s_set_freq_exclk(uint32_t freq, uint32_t exclk)
{
	uint32_t div;
	uint32_t temp;
	uint8_t wdwidth, stereo;
...
void uni_i2s_int_clear_all(void)
{
	tls_reg_write32(HR_I2S_INT_SRC, 0x3FF);
}

void uni_i2s_int_mask_all(void)
{
	tls_reg_write32(HR_I2S_INT_MASK, 0x3FF);
}

static void uni_i2s_dma_start(uint8_t ch)
{
	DMA_CHNLCTRL_REG(ch) = DMA_CHNL_CTRL_CHNL_ON;
}

#if 0
static void uni_i2s_dma_stop(uint8_t ch)
{
...
void uni_i2s_module_reset(void)
{
...
static void uni_i2s_dma_tx_init(uint8_t ch, uint32_t count, int16_t *buf)
{	
...
static void uni_i2s_dma_rx_init(uint8_t ch, uint32_t count, int16_t * buf)
{
...
{	
	...
	{	
		if (uni_i2s_buf->txtail < uni_i2s_buf->txlen)
		{
			for(fifo_level = ((I2S->INT_STATUS >> 4)& 0x0F),temp = 0; temp < 8-fifo_level; temp++)
			{
				tls_reg_write32(HR_I2S_TX, uni_i2s_buf->txbuf[uni_i2s_buf->txtail++]);
				if (uni_i2s_buf->txtail >= uni_i2s_buf->txlen)
				{
					uni_i2s_buf->txtail = 0;
					uni_i2s_buf->txdata_done = 1;
	...
	{		
		...
		{
			if (uni_i2s_buf->rxhead < uni_i2s_buf->rxlen)
			{
				uni_i2s_buf->rxbuf[uni_i2s_buf->rxhead++] = I2S->RX;
				if (uni_i2s_buf->rxhead >= uni_i2s_buf->rxlen)
				{
					uni_i2s_buf->rxhead = 0;
					uni_i2s_buf->rxdata_ready = 1;
...
{
	uni_dma_handler_type *hdma = (uni_dma_handler_type *)p;
	uni_dma_desc *dma_desc = &g_dma_desc_tx[0];
	if(p)
	{
		if(uni_i2s_buf->txdata_done)
		{
			uni_i2s_buf->txdata_done = 0;
...
{	
	uni_dma_handler_type *hdma = (uni_dma_handler_type *)p;
	uni_dma_desc *dma_desc = &g_dma_desc_rx[0];
	if(p)
	{
		if(uni_i2s_buf->rxdata_ready)
		{
			uni_i2s_buf->rxdata_ready = 0;
			dma_desc[1].valid |= (1 << 31);
      uni_i2s_dma_start(rx_channel);
			if(hdma->XferCpltCallback)
			{
				hdma->XferCpltCallback(hdma);
			}
...
void uni_i2s_register_callback(tls_i2s_callback callback)
{
	uni_i2s_buf->tx_callback = callback;
}

int uni_i2s_port_init(I2S_InitDef *opts)
{
	...
	uni_i2s_module_reset();
	
	tls_reg_write32(HR_I2S_CTRL, 0);
    uni_i2s_set_mode(opt.I2S_Mode_MS);
	
	uni_i2s_int_clear_all();
	uni_i2s_int_mask_all();

	uni_i2s_mono_select(opt.I2S_Mode_SS);
	uni_i2s_left_channel_sel(opt.I2S_Mode_LR);
	uni_i2s_set_format(opt.I2S_Trans_STD);
	uni_i2s_set_word_len(opt.I2S_DataFormat);
	uni_i2s_set_freq(opt.I2S_AudioFreq, opt.I2S_MclkFreq);
	uni_i2s_set_txth(4);
	uni_i2s_set_rxth(4);

//    tx_channel = tls_dma_request(UNI_I2S_TX_DMA_CHANNEL, TLS_DMA_FLAGS_CHANNEL_SEL(TLS_DMA_SEL_I2S_TX) | TLS_DMA_FLAGS_HARD_MODE);
//	rx_channel = tls_dma_request(UNI_I2S_RX_DMA_CHANNEL, TLS_DMA_FLAGS_CHANNEL_SEL(TLS_DMA_SEL_I2S_RX) | TLS_DMA_FLAGS_HARD_MODE);
//
...
void uni_i2s_tx_rx_stop(void)
{
//	if( I2S_MODE_MASTER==uni_i2s_get_mode() )
//	{
//		int i;
//
...
int uni_i2s_tx_int(int16_t *data, uint16_t len, int16_t *next_data)
{	
	volatile uint32_t temp;
	...
	uni_i2s_buf->txlen = (uint32_t)len;
	uni_i2s_buf->txtail = 0;
	uni_i2s_buf->txbuf = (uint32_t *)data;
	uni_i2s_buf->txdata_done = 0;

	uni_i2s_set_mode(I2S_MODE_MASTER);
	uni_i2s_txth_int_mask(0);
	uni_i2s_tx_fifo_clear();
	...
	if((uni_i2s_buf->tx_callback != NULL) && (next_data != NULL))
	{
		uni_i2s_buf->tx_callback((uint32_t *)next_data, &len);
	}
	while( uni_i2s_buf->txdata_done == 0 );
	while( tls_reg_read32(HR_I2S_STATUS)&0xF0 );
	
	return UNI_SUCCESS;
}

int uni_i2s_tx_dma(int16_t *data, uint16_t len, int16_t *next_data)
{		
	if((data == NULL) || (len == 0)) {
			return  UNI_FAILED;
	}
	...
	uni_i2s_set_mode(I2S_MODE_MASTER);
	uni_i2s_dma_tx_init(tx_channel, len*4, data);
	uni_i2s_dma_start(tx_channel);
...
int uni_i2s_tx_dma_link(int16_t *data, uint16_t len, int16_t *next_data)
{	
	if((data == NULL) || (next_data == NULL) || (len == 0)) {
			return  UNI_FAILED;
	}
	uint32_t dma_ctrl;
	uni_dma_desc dma_desc[2];
	...
	while(1)
	{
		...
		{
			dma_desc[0].valid = 0x80000000;
			if(uni_i2s_buf->tx_callback != NULL)
			{
				uni_i2s_buf->tx_callback((uint32_t *)data, &len);
			}
		}
...
int uni_i2s_rx_int(int16_t *data, uint16_t len)
{	
	if((data == NULL) || (len == 0)) {
			return  UNI_FAILED;
	}
	uni_i2s_buf->rxbuf = (uint32_t *)data;
	uni_i2s_buf->rxlen = (uint32_t)len;
	uni_i2s_buf->rxhead = 0;
	uni_i2s_buf->rxdata_ready = 0;

    uni_i2s_set_mode(I2S_MODE_SLAVE);	
	uni_i2s_rxth_int_mask(0);
	uni_i2s_rx_fifo_clear();
	tls_irq_enable(I2S_IRQn);
	uni_i2s_rx_enable(1);	
	uni_i2s_enable(1);
	
	while( uni_i2s_buf->rxdata_ready == 0 );

	return UNI_SUCCESS;
}

int uni_i2s_rx_dma(int16_t *data, uint16_t len)
{	
	if((data == NULL) || (len == 0)) {
			return  UNI_FAILED;
	}
	...
	uni_i2s_set_mode(I2S_MODE_SLAVE);
	uni_i2s_dma_rx_init(rx_channel, len*4, data);
	uni_i2s_dma_start(rx_channel);
	uni_i2s_rx_dma_enable(1);	
	uni_i2s_rx_enable(1);	
	uni_i2s_enable(1);
...
int uni_i2s_tx_rx_int(I2S_InitDef *opts, int16_t *data_tx, int16_t *data_rx, uint16_t len)
{
	if((data_tx == NULL) || (data_rx == NULL) || (len == 0)) {
		return  UNI_FAILED;
	}
	uni_i2s_buf->txbuf = (uint32_t *)data_tx;
	uni_i2s_buf->txlen = (uint32_t)len;
	uni_i2s_buf->txtail = 0;
	uni_i2s_buf->txdata_done = 0;

	uni_i2s_buf->rxbuf = (uint32_t *)data_rx;
	uni_i2s_buf->rxlen = (uint32_t)len;
	uni_i2s_buf->rxhead = 0;
	uni_i2s_buf->rxdata_ready = 0;
	
    uni_i2s_set_mode(opts->I2S_Mode_MS);
#if TEST_WITH_F401
	( opts->I2S_Trans_STD==I2S_Standard || opts->I2S_Trans_STD==I2S_Standard_MSB )?(uni_i2s_clk_inverse(0)):(uni_i2s_clk_inverse(1));
#endif
	uni_i2s_txth_int_mask(0);
    uni_i2s_rxth_int_mask(0);
	tls_irq_enable(I2S_IRQn);
	...
	while( uni_i2s_buf->txdata_done == 0 || uni_i2s_buf->rxdata_ready == 0);
	while( tls_reg_read32(HR_I2S_STATUS)&0xF0 );
	
	return UNI_SUCCESS;
}

int uni_i2s_tx_rx_dma(I2S_InitDef *opts, int16_t *data_tx, int16_t *data_rx, uint16_t len)
{	
	if((data_tx == NULL) || (data_rx == NULL) || (len == 0)) {
			return  UNI_FAILED;
	}
    ...
    uni_i2s_tx_enable(1);
	uni_i2s_rx_enable(1);
    uni_i2s_dma_tx_init(tx_channel, len*4, data_tx);
    uni_i2s_dma_rx_init(rx_channel, len*4, data_rx);
	uni_i2s_dma_start(tx_channel);
    uni_i2s_dma_start(rx_channel);
	uni_i2s_enable(1);
...
int uni_i2s_receive_dma(uni_dma_handler_type *hdma, uint16_t *data, uint16_t len)
{
	uint32_t dma_ctrl;
	uni_dma_desc *dma_desc = &g_dma_desc_rx[0];
...
int uni_i2s_transmit_dma(uni_dma_handler_type *hdma, uint16_t *data, uint16_t len)
{
	uint32_t dma_ctrl;
	uni_dma_desc *dma_desc = &g_dma_desc_tx[0];
	uint16_t *next_data = &data[(len/2)];
...
int uni_i2s_tranceive_dma(uint32_t i2s_mode, uni_dma_handler_type *hdma_tx, uni_dma_handler_type *hdma_rx, uint16_t *data_tx, uint16_t *data_rx, uint16_t len)
{
	uint32_t dma_ctrl;
	uni_dma_desc *dma_desc_tx = &g_dma_desc_tx[0];
	uni_dma_desc *dma_desc_rx = &g_dma_desc_rx[0];
...
void uni_i2s_play_demo(void)
{
	...
	uni_i2s_port_init(&i2s_config);
	uni_i2s_register_callback(i2s_demo_callback_play);
	memset(data_1[0], 1, sizeof(data_1[0]));
	memset(data_1[1], 2, sizeof(data_1[1]));
	
	printf("1\n");
	//uni_i2s_tx_int(data_1[0], UNIT_SIZE, data_1[1]);
	
	//uni_i2s_tx_dma(data_1[0], UNIT_SIZE, data_1[1]);
	
	//uni_i2s_tx_dma_link(data_1[0], UNIT_SIZE, data_1[1]);
	
	//uni_i2s_rx_int(data_1[0], UNIT_SIZE);
	//for( i=0; i<UNIT_SIZE; i++ ) { printf("%x\n", data_1[0][i]); }

	//uni_i2s_rx_dma(data_1[0], UNIT_SIZE);
	//for( i=0; i<UNIT_SIZE; i++ ) { printf("%x\n", data_1[0][i]); }

	//uni_i2s_tx_rx_int(&i2s_config, data_1[0], data_1[1], UNIT_SIZE);
	//for( i=0; i<UNIT_SIZE; i++ ) { printf("%x\n", data_1[1][i]); }

	uni_i2s_tx_rx_dma(&i2s_config, data_1[0], data_1[1], UNIT_SIZE);
	for( i=0; i<UNIT_SIZE; i++ ) { printf("%x\n", data_1[1][i]); }
