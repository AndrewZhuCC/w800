...
#include "uni_irq.h"
#include "uni_gpio.h"
#include "uni_hostspi.h"
#include "uni_dma.h"
#include "uni_dbg.h"
#include "uni_mem.h"
#include "uni_cpu.h"
#include "uni_spi_hal.h"
...
u32 spi_get_rxfifo(struct tls_spi_transfer *current_transfer, u32 current_remaining_bytes)
{
	...
	/*total rx length*/
#ifndef CONFIG_KERNEL_NONE	
	rx_len = current_transfer->len;
#else
	rx_len = current_transfer->rx_len;
#endif
    ...
    for (i = 0; i < rw_words; i++)
    {
        ...
        if (current_transfer->rx_buf)
        {
            if (SPI_BYTE_TRANSFER == spi_port->transtype)
            {
                ...
                (((u8 *)current_transfer->rx_buf + (rx_len - rx_remaining_bytes)))[0] = (u8)data32;
                (((u8 *)current_transfer->rx_buf + (rx_len - rx_remaining_bytes)))[1] = (u8)(data32 >> 8);
                (((u8 *)current_transfer->rx_buf + (rx_len - rx_remaining_bytes)))[2] = (u8)(data32 >> 16);
                (((u8 *)current_transfer->rx_buf + (rx_len - rx_remaining_bytes)))[3] = (u8)(data32 >> 24);
            }
            else if (SPI_WORD_TRANSFER == spi_port->transtype)
            {
                *((u32 *)((u8 *)current_transfer->rx_buf + rx_len - rx_remaining_bytes)) = data32;
            }
        }
    ...
    if (rw_bytes)
    {
        ...
        if (current_transfer->rx_buf)
        {
            for (i = 0; i < rw_bytes; i++)
            {
                ...
                ((u8 *)current_transfer->rx_buf)[rx_len - rx_remaining_bytes] = data8;
...
int tls_spi_read_with_cmd(const u8 *txbuf, u32 n_tx, u8 *rxbuf, u32 n_rx)
{
    ...
    return status;
#else
	return tls_spi_xfer(txbuf, rxbuf, n_tx, n_rx + n_tx);
#endif	
}
