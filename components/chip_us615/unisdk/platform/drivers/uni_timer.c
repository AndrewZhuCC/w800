 ...
 *
 * @retval         	UNI_TIMER_ID_INVALID     failed
...
u8 tls_timer_create(struct tls_timer_cfg *cfg)
{
    ...
    {
        if (!(uni_timer_bitmap & BIT(i)))
            break;
    }

    if (TLS_TIMER_ID_MAX == i)
        return UNI_TIMER_ID_INVALID;

    uni_timer_bitmap  |= BIT(i);
...
{
    if (!(uni_timer_bitmap & BIT(timer_id)))
        return;
