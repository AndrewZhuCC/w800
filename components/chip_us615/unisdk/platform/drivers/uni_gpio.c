...
static struct gpio_irq_context gpio_context[UNI_IO_PB_31 - UNI_IO_PA_00 + 1] = {{0,0}};
...
void __GPIOB_IRQHandler(void)
{
    ...
    for (i = UNI_IO_PB_00; i <= UNI_IO_PB_31; i++)
    {
        if (reg & BIT(i - UNI_IO_PB_00))
        {
            found = 1;
            break;
        }
 ...
 *
 * @note
 * From gpio3 to gpio7,attribute can set to UNI_GPIO_ATTR_PULLHIGH,
 * but can not set to UNI_GPIO_ATTR_PULLLOW,
 * the default attribute is UNI_GPIO_ATTR_PULLHIGH.
 * Other gpio can set to UNI_GPIO_ATTR_PULLLOW,but can not set to UNI_GPIO_ATTR_PULLHIGH,the deault
 * attribute is UNI_GPIO_ATTR_PULLLOW.
 * all gpio can set to UNI_GPIO_ATTR_FLOATING
 */
void tls_gpio_cfg(enum tls_io_name gpio_pin, enum tls_gpio_dir dir, enum tls_gpio_attr attr)
{
    ...
    if (gpio_pin >= UNI_IO_PB_00)
    {
        pin    = gpio_pin - UNI_IO_PB_00;
	...
	if (UNI_GPIO_ATTR_PULLHIGH == attr)
	{
...
void tls_gpio_cfg_attr(enum tls_io_name gpio_pin, enum tls_gpio_attr attr)
{
    ...
    else if (UNI_GPIO_ATTR_PULLHIGH == attr)
    {
        tls_reg_write32(HR_GPIO_PULLUP_EN + offset, tls_reg_read32(HR_GPIO_PULLUP_EN + offset) & (~BIT(pin)));		/* 0 enable pullup */
		tls_reg_write32(HR_GPIO_PULLDOWN_EN + offset, tls_reg_read32(HR_GPIO_PULLDOWN_EN + offset) &(~BIT(pin)));	   /* 0 disable pulldown */
    }
    else if (UNI_GPIO_ATTR_PULLLOW == attr)
	{
        tls_reg_write32(HR_GPIO_PULLUP_EN + offset, tls_reg_read32(HR_GPIO_PULLUP_EN + offset) | BIT(pin)); 	/* 0 disable pullup */
		tls_reg_write32(HR_GPIO_PULLDOWN_EN + offset, tls_reg_read32(HR_GPIO_PULLDOWN_EN + offset) | BIT(pin)); 	   /* 1 disable pulldown */
	}
...
//	TLS_DBGPRT_INFO("\r\ntls_gpio_int_enable gpio pin =%d,mode==%d\r\n",gpio_pin,mode);
	...
	switch(mode)
	{
		case UNI_GPIO_IRQ_TRIG_RISING_EDGE:
		...
		//	TLS_DBGPRT_INFO("\r\nrising edge iev ret=%x\r\n",reg);
			...
			break;
		case UNI_GPIO_IRQ_TRIG_FALLING_EDGE:
			reg = tls_reg_read32(HR_GPIO_IS + offset);
			reg &= (~(0x1 << pin));
		...
		//	TLS_DBGPRT_INFO("\falling edge iev ret=%x\n",reg);
			...
			break;
		case UNI_GPIO_IRQ_TRIG_DOUBLE_EDGE:
			reg = tls_reg_read32(HR_GPIO_IS + offset);
			...
			break;
		case UNI_GPIO_IRQ_TRIG_HIGH_LEVEL:
			reg = tls_reg_read32(HR_GPIO_IS + offset);
			...
			break;
		case UNI_GPIO_IRQ_TRIG_LOW_LEVEL:
			reg = tls_reg_read32(HR_GPIO_IS + offset);
			tls_reg_write32(HR_GPIO_IS + offset, reg | (0x1 << pin));		/* 1 level trigger */
			reg = tls_reg_read32(HR_GPIO_IEV + offset);
