...
int tls_pwm_out_mode_config(u8 channel, enum tls_pwm_out_mode mode)
{
    ...
    else if (UNI_PWM_OUT_MODE_2SYC == mode)
    {
    ...
    else if (UNI_PWM_OUT_MODE_MC == mode)
    {
        if (channel != 0 && channel != 2)
            return UNI_FAILED;
        tls_reg_write32(HR_PWM_BRKCTL, tls_reg_read32(HR_PWM_BRKCTL) & ~(0x1800<<channel));/* disable the brake mode */
    ...
    else if(UNI_PWM_OUT_MODE_INDPT == mode)
    {
...
int tls_pwm_cnt_type_config(u8 channel, enum tls_pwm_cnt_type cnt_type)
{
    ...
    if (4 == channel)
    {
        if (UNI_PWM_CNT_TYPE_EDGE_ALLGN_CAP == cnt_type)
        {
        ...
        if (UNI_PWM_CNT_TYPE_EDGE_ALIGN_OUT == cnt_type)
        {
            tls_reg_write32(HR_PWM_CH4_REG2, tls_reg_read32(HR_PWM_CH4_REG2) & (~BIT(4)));
            tls_reg_write32(HR_PWM_CH4_REG2, tls_reg_read32(HR_PWM_CH4_REG2) |   BIT(3));
        }
        else if (UNI_PWM_CNT_TYPE_CENTER_ALIGN == cnt_type)
        {
            tls_reg_write32(HR_PWM_CH4_REG2, tls_reg_read32(HR_PWM_CH4_REG2) |   BIT(4));
            tls_reg_write32(HR_PWM_CH4_REG2, tls_reg_read32(HR_PWM_CH4_REG2) & (~BIT(3)));
        }
    ...
    {
        if (UNI_PWM_CNT_TYPE_EDGE_ALLGN_CAP == cnt_type && channel == 0)
        {
...
int tls_pwm_loop_mode_config(u8 channel, enum tls_pwm_loop_type loop_mode)
{
    ...
    if (4 == channel)
    {
        if (UNI_PWM_LOOP_TYPE_LOOP == loop_mode)
...
int tls_pwm_brake_mode_config(u8 channel, bool en, enum tls_pwm_brake_out_level brok)
{
    ...
    if (ENABLE == en)
    {
        if (UNI_PWM_BRAKE_OUT_HIGH == brok)
...
int tls_pwm_capture_irq_type_config(u8 channel, enum tls_pwm_cap_int_type int_type)
{
    ...
    if (0 == channel)
    {
        if (UNI_PWM_CAP_RISING_FALLING_EDGE_INT == int_type)
        {
        ...
        else if (UNI_PWM_CAP_RISING_EDGE_INT == int_type)
        {
            tls_reg_write32(HR_PWM_INTEN, tls_reg_read32(HR_PWM_INTEN) |   BIT(5));
            tls_reg_write32(HR_PWM_INTEN, tls_reg_read32(HR_PWM_INTEN) & (~BIT(6)));
            tls_reg_write32(HR_PWM_INTEN, tls_reg_read32(HR_PWM_INTEN) & (~BIT(7)));
        }
        else if (UNI_PWM_CAP_FALLING_EDGE_INT == int_type)
        {
            tls_reg_write32(HR_PWM_INTEN, tls_reg_read32(HR_PWM_INTEN) |   BIT(6));
            tls_reg_write32(HR_PWM_INTEN, tls_reg_read32(HR_PWM_INTEN) & (~BIT(5)));
            tls_reg_write32(HR_PWM_INTEN, tls_reg_read32(HR_PWM_INTEN) & (~BIT(7)));
        }
        else if(UNI_PWM_CAP_DMA_INT == int_type)
        {
            tls_reg_write32(HR_PWM_INTEN, tls_reg_read32(HR_PWM_INTEN) |   BIT(7));
            tls_reg_write32(HR_PWM_INTEN, tls_reg_read32(HR_PWM_INTEN) & (~BIT(5)));
            tls_reg_write32(HR_PWM_INTEN, tls_reg_read32(HR_PWM_INTEN) & (~BIT(6)));
...
int tls_pwm_out_init(pwm_init_param * pwm_param)
{
    ...
    if (UNI_PWM_OUT_MODE_MC == pwm_param->mode)
    {
        /* set dead time */
    ...
    if (pwm_param->channel == 0 || pwm_param->channel == 4)
        tls_pwm_output_en_cmd(pwm_param->channel, UNI_PWM_OUT_EN_STATE_OUT);
    if (pwm_param->mode == UNI_PWM_OUT_MODE_ALLSYC && pwm_param->channel == 0)
        tls_pwm_output_en_cmd(4, UNI_PWM_OUT_EN_STATE_OUT);
    return UNI_SUCCESS;
}
...
int tls_pwm_cap_init(u8 channel, u16 clkdiv, bool inverse_en, enum tls_pwm_cap_int_type int_type)
{
    ...
    tls_pwm_cnt_type_config(channel, UNI_PWM_CNT_TYPE_EDGE_ALLGN_CAP);

    /* set output status */
    if(channel == 0)
        tls_pwm_output_en_cmd(channel, UNI_PWM_OUT_EN_STATE_TRI);
...
int tls_pwm_init(u8 channel,u32 freq, u8 duty, u8 pnum)
{
    ...
    pwm_param.cnt_type = UNI_PWM_CNT_TYPE_EDGE_ALIGN_OUT;
    pwm_param.loop_type = UNI_PWM_LOOP_TYPE_LOOP;
    pwm_param.mode = UNI_PWM_OUT_MODE_INDPT;
