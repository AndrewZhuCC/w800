...
#include "uni_list.h"
#include "uni_regs.h"
#include "uni_internal_flash.h"
#include "uni_flash_map.h"
...
// unsigned int  TLS_FLASH_PARAM_RESTORE_ADDR   =      (0x81FC000UL);
unsigned int  TLS_FLASH_PARAM1_ADDR          =      (0x81FD000UL);
unsigned int  TLS_FLASH_PARAM2_ADDR          =      (0x81FE000UL);
...
static int __innerreadbycmd(unsigned char cmd, unsigned long addr, unsigned char *buf, unsigned long sz)
{
  int i = 0;
  int word = sz / 4;
  int byte = sz % 4;
...
int flashRead(unsigned long addr, unsigned char *buf, unsigned long sz)
{
  ...
  if (cache == NULL)
  {
    LOGE(TAG, "allocate sector cache memory fail!\n");
    return TLS_FLS_STATUS_ENOMEM;
  }
  flash_addr = addr & ~(INSIDE_FLS_PAGE_SIZE - 1);
  __innerreadbycmd(0xEB, flash_addr, (unsigned char *)cache, INSIDE_FLS_PAGE_SIZE);
  if (sz > INSIDE_FLS_PAGE_SIZE - page_offset)
  {
    ...
    if (sz_remain)
    {
      __innerreadbycmd(0xEB, flash_addr, (unsigned char *)cache,
                       sz_remain + (4 - sz_remain % 4));
...
int tls_fls_read(u32 addr, u8 *buf, u32 len)
{
    ...
    if (inside_fls == NULL)
    {
        LOGE(TAG, "flash driver module not beed installed!");
        return TLS_FLS_STATUS_EPERM;
    }
#endif
    if (((addr & (INSIDE_FLS_BASE_ADDR - 1)) >=  flsdensity) || (len == 0) || (buf == NULL))
    {
        LOGE(TAG, "addr[0x%x], len[%d], buf is NULL?[%d], getFlashDensity[0x%x]", addr, len,
             buf == NULL, flsdensity);
  ...
  tls_os_sem_acquire(inside_fls->fls_lock, 0);
    ...
    if (err != 0) {
        LOGE(TAG, "flashRead failed, err[%d], addr[0x%x], len[%d]", err, addr, len);
    }
...
int readByCMD(unsigned char cmd, unsigned long addr, unsigned char *buf, unsigned long sz)
{
  ...
  tls_os_sem_acquire(inside_fls->fls_lock, 0);
#endif
  __innerreadbycmd(cmd, addr, buf, sz);
...
void tls_fls_sys_param_postion_init(void)
{
  ...
  if (TLS_FLS_STATUS_OK == err)
  {
    ...
    TLS_FLASH_PARAM2_ADDR         = (FLASH_BASE_ADDR|density) - 0x2000;
    TLS_FLASH_PARAM1_ADDR         = (FLASH_BASE_ADDR|density) - 0x3000;
    // TLS_FLASH_PARAM_RESTORE_ADDR  = (FLASH_BASE_ADDR|density) - 0x4000;
