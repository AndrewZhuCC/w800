...
static err_t us615_netif_init(struct netif *netif)
{
  LWIP_ASSERT("netif != NULL", (netif != NULL));
#if LWIP_NETIF_HOSTNAME
	/* Initialize interface hostname */
	netif->hostname = "us615-wifi";
...
static void us615_netif_status_changed(struct netif *netif)
{
...
static int us615_netif_input(const u8 *bssid, u8 *buf, u32 buf_len)
{
    struct netif    *netif = &us615_netif[0];
    struct pbuf       *p;
...
static void us615_lwip_init(void)
{
    tcpip_init(NULL, NULL);

    us615_netif[0].name[0] = 'w';
    us615_netif[0].name[1] = '0';
    netifapi_netif_add(&us615_netif[0], IPADDR_ANY, IPADDR_ANY, IPADDR_ANY, NULL, us615_netif_init, tcpip_input);
    netif_set_status_callback(&us615_netif[0], us615_netif_status_changed);

    us615_netif[1].name[0] = 'w';
    us615_netif[1].name[1] = '1';
    netifapi_netif_add(&us615_netif[1], IPADDR_ANY, IPADDR_ANY, IPADDR_ANY, NULL, us615_netif_init, tcpip_input);
    //netif_set_status_callback(&us615_netif[1], us615_netif_status_changed);

    netifapi_netif_set_default(&us615_netif[0]);

    netifapi_netif_set_up(&us615_netif[0]);
    netifapi_netif_set_up(&us615_netif[1]);
}

static void uni_wlan_auto_reconnect(void)
{
...
static void uni_wlan_client_event(u8 *mac, enum tls_wifi_client_event_type event)
{
    LOGD(TAG, "sta "MACSTR" %s", MAC2STR(mac), event ? "offline" : "online");
}

static void uni_wlan_status_changed(uint8_t status)
{
    ...
    switch (status)
    {
        ...
        case WIFI_SOFTAP_SUCCESS:
            ...
            netif_set_addr(&us615_netif[1], &ipaddr, &netmask, &gw);
            dhcps_deinit();
            ...
            dhcps_init(&us615_netif[1]);
            netif_set_link_up(&us615_netif[1]);
            break;
        ...
        case WIFI_SOFTAP_CLOSED:
            ...
            netif_set_link_down(&us615_netif[1]);
            dhcps_deinit();
            break;
...
static void uni_wlan_scan_callback(void)
{
    int err;
    int buflen = 2000;
    char *buf = NULL;
    ...
    if (g_evt_func)
    {
        if (g_evt_func->scan_compeleted)
        {
            ...
            for (i = 0; i < scan_res->count; i ++)
            {
                ...
                if (UNI_WIFI_DISCONNECTED != tls_wifi_get_state())
                {
                ...
                if (((UNI_WIFI_AUTH_MODE_WPA_PSK_TKIP & bss_info->privacy) && (UNI_WIFI_AUTH_MODE_WPA_PSK_CCMP & bss_info->privacy)) ||
                    ((UNI_WIFI_AUTH_MODE_WPA2_PSK_TKIP & bss_info->privacy) && (UNI_WIFI_AUTH_MODE_WPA2_PSK_CCMP & bss_info->privacy)))
                {
                    ap_records->encryptmode = WIFI_ENC_TKIP_AES_MIX;
                }
                else if ((UNI_WIFI_AUTH_MODE_WPA_PSK_CCMP & bss_info->privacy) || (UNI_WIFI_AUTH_MODE_WPA2_PSK_CCMP & bss_info->privacy))
                {
                    ap_records->encryptmode = WIFI_ENC_AES;
                }
                else if ((UNI_WIFI_AUTH_MODE_WPA_PSK_TKIP & bss_info->privacy) || (UNI_WIFI_AUTH_MODE_WPA2_PSK_TKIP & bss_info->privacy))
                {
                    ap_records->encryptmode = WIFI_ENC_TKIP;
                }
                if (UNI_WIFI_AUTH_MODE_WEP_AUTO & bss_info->privacy)
                {
                    ap_records->encryptmode = WIFI_ENC_WEP;
                }

                if ((UNI_WIFI_AUTH_MODE_WPA_PSK_AUTO & bss_info->privacy) && 
                    (UNI_WIFI_AUTH_MODE_WPA2_PSK_AUTO & bss_info->privacy))
                {
                    ap_records->authmode = WIFI_AUTH_WPA_WPA2_PSK;
                }
                else if (UNI_WIFI_AUTH_MODE_WPA2_PSK_AUTO & bss_info->privacy)
                {
                    ap_records->authmode = WIFI_AUTH_WPA2_PSK;
                }
                else if (UNI_WIFI_AUTH_MODE_WPA_PSK_AUTO & bss_info->privacy)
                {
                    ap_records->authmode = WIFI_AUTH_WPA_PSK;
                }

                if (UNI_WIFI_AUTH_MODE_OPEN == bss_info->privacy)
                {
                    ap_records->encryptmode = WIFI_ENC_OPEN;
                    ap_records->authmode = WIFI_AUTH_OPEN;
                }
                if (UNI_WIFI_AUTH_MODE_UNKNOWN == bss_info->privacy)
                {
...
static void uni_wlan_data_recv_callback(u8* data, u32 data_len)
{
...
static int us615_set_mac_addr(aos_dev_t *dev, const uint8_t *mac)
{
...
static int us615_get_mac_addr(aos_dev_t *dev, uint8_t *mac)
{
...
static int us615_set_dns_server(aos_dev_t *dev, ip_addr_t ipaddr[], uint32_t num)
{
    int n, i;
...
static int us615_get_dns_server(aos_dev_t *dev, ip_addr_t ipaddr[], uint32_t num)
{
    int n, i;
...
static int us615_set_hostname(aos_dev_t *dev, const char *name)
{
#if LWIP_NETIF_HOSTNAME
    struct netif *netif = &us615_netif[0];
...
static const char *us615_get_hostname(aos_dev_t *dev)
{
#if LWIP_NETIF_HOSTNAME
    struct netif *netif = &us615_netif[0];
...
static int us615_set_link_up(aos_dev_t *dev)
{
    //netif_set_link_up(&us615_netif[0]);
    return 0;
}

static int us615_set_link_down(aos_dev_t *dev)
{
    //netif_set_link_down(&us615_netif[0]);
    return 0;
}

static int us615_start_dhcp(aos_dev_t *dev)
{
    struct netif *netif = &us615_netif[0];
...
static int us615_stop_dhcp(aos_dev_t *dev)
{
    struct netif *netif = &us615_netif[0];
    aos_check_return_einval(netif);
...
static int us615_set_ipaddr(aos_dev_t *dev, const ip_addr_t *ipaddr, const ip_addr_t *netmask,
                           const ip_addr_t *gw)
{
    struct netif *netif = &us615_netif[0];
    ...
    us615_netif_status_changed(netif);

    return 0;
}

static int us615_get_ipaddr(aos_dev_t *dev, ip_addr_t *ipaddr, ip_addr_t *netmask, ip_addr_t *gw)
{
    struct netif *netif = &us615_netif[0];
...
static int us615_ping_remote(aos_dev_t *dev, int type, char *remote_ip)
{
    return -1;
}

static int us615_subscribe(aos_dev_t *dev, uint32_t event, event_callback_t cb, void *param)
{
    return -1;
}

static int us615_unsubscribe(aos_dev_t *dev, uint32_t event, event_callback_t cb, void *param)
{
    return -1;
}

static net_ops_t us615_net_driver = {
    .set_mac_addr   = us615_set_mac_addr,
    .get_mac_addr   = us615_get_mac_addr,

    .set_dns_server = us615_set_dns_server,
    .get_dns_server = us615_get_dns_server,

    .set_hostname   = us615_set_hostname,
    .get_hostname   = us615_get_hostname,

    .set_link_up    = us615_set_link_up,
    .set_link_down  = us615_set_link_down,

    .start_dhcp     = us615_start_dhcp,
    .stop_dhcp      = us615_stop_dhcp,
    .set_ipaddr     = us615_set_ipaddr,
    .get_ipaddr     = us615_get_ipaddr,
    .ping           = us615_ping_remote,

    .subscribe      = us615_subscribe,
    .unsubscribe    = us615_unsubscribe,
};

static int us615_init(aos_dev_t *dev)
{
    ...
    if (!wifi_inited)
    {
        ...
        if (tls_wifi_mem_cfg(WIFI_MEM_START_ADDR, 5, 3))
        {
        ...
        tls_wifi_status_change_cb_register(uni_wlan_status_changed);
        tls_wifi_softap_client_event_register(uni_wlan_client_event);
        tls_ethernet_data_rx_callback(us615_netif_input);
...
static int us615_deinit(aos_dev_t *dev)
{
    tls_wifi_disconnect();
...
static int us615_reset(aos_dev_t *dev)
{
    tls_wifi_disconnect();
    tls_wifi_softap_destroy();
...
static int us615_set_mode(aos_dev_t *dev, wifi_mode_t mode)
{
...
static int us615_get_mode(aos_dev_t *dev, wifi_mode_t *mode)
{
    u8 wmode = IEEE80211_MODE_INFRA;
...
static int us615_install_event_cb(aos_dev_t *dev, wifi_event_func *evt_func)
{
    g_evt_func   = evt_func;
    wifi_evt_dev = dev;
...
static int us615_set_protocol(aos_dev_t *dev, uint8_t protocol_bitmap)
{
...
static int us615_get_protocol(aos_dev_t *dev, uint8_t *protocol_bitmap)
{
    struct tls_param_bgr bgr;
...
static int us615_set_country(aos_dev_t *dev, wifi_country_t country)
{
...
static int us615_get_country(aos_dev_t *dev, wifi_country_t *country)
{
...
static int us615_set_auto_reconnect(aos_dev_t *dev, bool en)
{
#if 0
    u8 mode;
...
static int us615_get_auto_reconnect(aos_dev_t *dev, bool *en)
{
#if 0
    u8 mode;
...
static int us615_set_lpm(aos_dev_t *dev, wifi_lpm_mode_t mode)
{
...
static int us615_get_lpm(aos_dev_t *dev, wifi_lpm_mode_t *mode)
{
    *mode = g_wifi_lpm_mode;
    return WIFI_ERR_OK;
}

static int us615_power_on(aos_dev_t *dev)
{
    tls_wl_if_ps(1);
    return WIFI_ERR_OK;
}

static int us615_power_off(aos_dev_t *dev)
{
    tls_wl_if_ps(0);
    return WIFI_ERR_OK;
}

static int us615_start_scan(aos_dev_t *dev, wifi_scan_config_t *config, bool block)
{
    int ret;
    ...
    tls_wifi_scan_result_cb_register(uni_wlan_scan_callback);

    while (UNI_SUCCESS != tls_wifi_scan())
	{
...
static int us615_start_sta(wifi_config_t *config)
{
    int ret;
    u8 channel_en;
    u8 channel;
...
static int us615_start_softap(wifi_config_t *config)
{
    int ret;
    ...
    ipinfo->netmask[3] = 0;
    memcpy(ipinfo->dnsname, "local.uni", sizeof("local.uni"));
...
static int us615_start(aos_dev_t *dev, wifi_config_t *config)
{
    int ret;
...
static int us615_stop(aos_dev_t *dev)
{
    u8 wmode = IEEE80211_MODE_INFRA;
...
static int us615_sta_get_link_status(aos_dev_t *dev, wifi_ap_record_t *ap_info)
{
    struct tls_curr_bss_t currbss;
...
static int us615_ap_get_sta_list(aos_dev_t *dev, wifi_sta_list_t *sta)
{
...
int us615_start_mgnt_monitor(aos_dev_t *dev, wifi_mgnt_cb_t cb)
{
...
int us615_stop_mgnt_monitor(aos_dev_t *dev)
{
    return 0;
}

static int us615_start_monitor(aos_dev_t *dev, wifi_promiscuous_cb_t cb)
{
    if (!cb)
        return WIFI_ERR_ARG;
    ...
    tls_wifi_data_recv_cb_register(uni_wlan_data_recv_callback);
    tls_wl_plcp_cb_register(uni_wlan_data_recv_callback);
...
static int us615_stop_monitor(aos_dev_t *dev)
{
...
static int us615_send_80211_raw_frame(aos_dev_t *dev, void *buffer, uint16_t len)
{
    ...
    tx.tx_rate = UNI_WIFI_TX_RATEIDX_36M;
...
static int us615_set_channel(aos_dev_t *dev, uint8_t primary, wifi_second_chan_t second)
{
...
static int us615_get_channel(aos_dev_t *dev, uint8_t *primary, wifi_second_chan_t *second)
{
    *primary = hed_rf_current_channel;
    return WIFI_ERR_OK;
}

static int us615_set_smartcfg(aos_dev_t *dev, int enable)
{
...
static wifi_driver_t us615_wifi_driver = {
    .init             = us615_init,
    .deinit           = us615_deinit,
    .reset            = us615_reset,
    .set_mode         = us615_set_mode,
    .get_mode         = us615_get_mode,
    .install_event_cb = us615_install_event_cb,

    .set_protocol        = us615_set_protocol,
    .get_protocol        = us615_get_protocol,
    .set_country         = us615_set_country,
    .get_country         = us615_get_country,
    .set_mac_addr        = us615_set_mac_addr,
    .get_mac_addr        = us615_get_mac_addr,
    .set_auto_reconnect  = us615_set_auto_reconnect,
    .get_auto_reconnect  = us615_get_auto_reconnect,
    .set_lpm             = us615_set_lpm,
    .get_lpm             = us615_get_lpm,
    .power_on            = us615_power_on,
    .power_off           = us615_power_off,

    .start_scan          = us615_start_scan,
    .start               = us615_start,
    .stop                = us615_stop,
    .sta_get_link_status = us615_sta_get_link_status,
    .ap_get_sta_list     = us615_ap_get_sta_list,

    .start_monitor        = us615_start_monitor,
    .stop_monitor         = us615_stop_monitor,
    .start_mgnt_monitor   = us615_start_mgnt_monitor,
    .stop_mgnt_monitor    = us615_stop_mgnt_monitor,
    .send_80211_raw_frame = us615_send_80211_raw_frame,
    .set_channel          = us615_set_channel,
    .get_channel          = us615_get_channel,

    .set_smartcfg         = us615_set_smartcfg,
};

static aos_dev_t *us615_dev_init(driver_t *drv, void *config, int id)
{
...
static void us615_dev_uninit(aos_dev_t *dev)
{
    aos_check_param(dev);
...
static int us615_dev_open(aos_dev_t *dev)
{
    return 0;
}

static int us615_dev_close(aos_dev_t *dev)
{
    return 0;
}

static netdev_driver_t us615_driver = {
    .drv =
        {
            .name   = "wifi",
            .init   = us615_dev_init,
            .uninit = us615_dev_uninit,
            .open   = us615_dev_open,
            .close  = us615_dev_close,
        },
    .link_type = NETDEV_TYPE_WIFI,
    .net_ops   = &us615_net_driver,
    .link_ops  = &us615_wifi_driver,
};
...
static void us615_pin_init(us615_wifi_param_t *config)
{
...
{
    if (UNI_WIFI_JOINED == tls_wifi_get_state()) {
        return 1;
    } else {
        return 0;
    }
...
void wifi_us615_register(us615_wifi_param_t *config)
{
    ...
    if (config)
        us615_pin_init(config);

    us615_lwip_init();

    driver_register(&us615_driver.drv, NULL, 0);
}
