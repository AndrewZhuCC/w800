...
static int hash_start(sha_context_t *ctx)
{
    ...
    sc_sha_start(&ctx->sc_sha, &ctx->sc_ctx, mode);
    return 0;
}
...
static int hash_update(sha_context_t *ctx, const void *input, uint32_t ilen, int from_mem)
{
    ...
    if (image_size > sizeof(temp_buffer)) {
        ...
        sc_sha_update(&ctx->sc_sha, &ctx->sc_ctx, temp_buffer, sizeof(temp_buffer));
        ...
        sc_sha_update(&ctx->sc_sha, &ctx->sc_ctx, temp_buffer, image_size);
    } else {
        copy_data(temp_buffer, p_addr, image_size, from_mem);
        sc_sha_update(&ctx->sc_sha, &ctx->sc_ctx, temp_buffer, image_size);
    }
...
{
    sc_sha_finish(&ctx->sc_sha, &ctx->sc_ctx, output, olen);
    return 0;
}
